...
typeAliases :: IORef (Map Text HLIR.Type)
typeAliases = IO.unsafePerformIO $ newIORef Map.empty
...
convertSingularNode (HLIR.MkTopExternalFunction header typeValue) = do
  modifyIORef' natives (Map.insert header.name typeValue)
  pure [HLIR.MkTopExternalFunction header typeValue]
...
convertSingularNode (HLIR.MkTopEnumeration header constructors) = do
  ...
  funcs <- concat <$> mapM (createFunction header.name) (Map.toList constructors)

  pure $ enum : funcs
  where
    createEnumStructure ::
      (MonadIO m) => (Text, Map Text (Maybe [HLIR.Type])) -> m (HLIR.TLIR "toplevel")
      ...
      namedFields <- forM (Map.toList fields) $ \(name, fieldTypes) -> do
        let constructorFields =
              List.foldr
                ( \(ty, idx) acc ->
                    HLIR.MkTyRowExtend
                      (name <> "_field" <> Text.pack (show idx))
                      ty
                      False
                      acc
                )
                HLIR.MkTyRowEmpty
                (zip fieldTypes [(1 :: Int) ..])

        pure (name, HLIR.MkTyRecord constructorFields)

      let typeValue = HLIR.MkTyRecord (HLIR.MkTyRowExtend "_tag" HLIR.MkTyString False (HLIR.MkTyRowExtend "_data" (HLIR.MkTyUnion (Map.fromList namedFields)) False HLIR.MkTyRowEmpty))

      let enumStructure =
            HLIR.MkTopTypeAlias
              (HLIR.MkAnnotation enumName [])
              typeValue

      -- modifyIORef' structureDeclarations (enumStructure :)
      modifyIORef' typeAliases (Map.insert enumName typeValue)

      pure enumStructure

    createFunction ::
      (MonadIO m) => Text -> (Text, Maybe [HLIR.Type]) -> m [HLIR.TLIR "toplevel"]
    createFunction enumName (consName, maybeFieldTypes) = do
      let arguments = case maybeFieldTypes of
            Just fieldTypes ->
              zipWith
                (\ty idx -> HLIR.MkAnnotation ("arg" <> Text.pack (show idx)) ty)
                fieldTypes
                [(1 :: Int) ..]
            Nothing -> []

          structType = HLIR.MkTyId enumName

          resultVar = HLIR.MkAnnotation "result" (Identity structType)

          createUpdatedField :: [(HLIR.Annotation HLIR.Type, Int)] -> HLIR.TLIR "expression"
          createUpdatedField ((f, i) : fields) =
            HLIR.MkExprLetIn
              (HLIR.MkAnnotation "_" (Identity f.typeValue))
              ( HLIR.MkExprUpdate
                  ( HLIR.MkExprStructureAccess
                      ( HLIR.MkExprStructureAccess
                          (HLIR.MkExprStructureAccess (HLIR.MkExprVariable resultVar []) "_data")
                          consName
                      )
                      (consName <> "_field" <> Text.pack (show i))
                  )
                  (HLIR.MkExprVariable (Identity <$> f) [])
                  (Identity structType)
              )
              (createUpdatedField fields)
              (Identity structType)
          createUpdatedField [] = HLIR.MkExprVariable resultVar []

          bodyExpr =
            HLIR.MkExprLetValueless
              (runIdentity <$> resultVar)
              ( HLIR.MkExprLetIn
                  (HLIR.MkAnnotation "_" (Identity (HLIR.MkTyPointer HLIR.MkTyChar)))
                  ( HLIR.MkExprUpdate
                      (HLIR.MkExprStructureAccess (HLIR.MkExprVariable resultVar []) "_tag")
                      (HLIR.MkExprLiteral (HLIR.MkLitString consName))
                      (Identity structType)
                  )
                  (createUpdatedField (zip arguments [(1 :: Int) ..]))
                  (Identity structType)
              )

      case arguments of
        (x : xs) -> do
          let funReturnTypes = map (.typeValue) xs
          let lambda =
                List.foldl'
                  ( \acc (HLIR.MkAnnotation argName argType, retType) ->
                      HLIR.MkExprLambda
                        { HLIR.parameters = [HLIR.MkAnnotation argName (Identity argType)],
                          HLIR.returnType = Identity retType,
                          HLIR.body = acc
                        }
                  )
                  bodyExpr
                  (zip xs (funReturnTypes ++ [structType]))
          let funTypes = funReturnTypes

          let funType = List.foldr (HLIR.MkTyFun . (: [])) structType funTypes

          convertSingularNode $
            HLIR.MkTopFunctionDeclaration
              (HLIR.MkAnnotation consName [])
              [x]
              funType
              lambda
        [] ->
...
convertExpression (HLIR.MkExprLetIn binding value inExpr _)
  ...
  | Just (HLIR.MkExprLambda args returnType body) <- getLambda value = do
      ...
      (newInExpr, ns3, retTy) <- convertExpression inExpr
...
convertExpression (HLIR.MkExprApplication callee arguments retType) = do
  ...
  case getVariable callee of
    ...
    _ -> do
      ...
      let call = HLIR.MkExprApplication function (environment : newArguments) (Identity retType')
...
convertExpression (HLIR.MkExprStructureAccess struct field) = do
  ...
  m <- resolveStructureType structTy
  ...
  where
    resolveStructureType :: (MonadIO m) => HLIR.Type -> m (Map Text.Text HLIR.Type)
    resolveStructureType (HLIR.MkTyRecord fields) = resolveStructureType fields
    resolveStructureType HLIR.MkTyRowEmpty = pure Map.empty
    resolveStructureType (HLIR.MkTyUnion fields) = do
      fieldTypes <- mapM resolveStructureType fields
      pure $ Map.unions fieldTypes <> fields
    resolveStructureType (HLIR.MkTyRowExtend label fieldType _ rest) = do
      restFields <- resolveStructureType rest
      fieldType' <- resolveStructureType fieldType
      pure $ Map.insert label fieldType restFields <> fieldType'
    resolveStructureType (HLIR.MkTyId typeName) = do
      aliases <- readIORef typeAliases
      case Map.lookup typeName aliases of
        Just ty -> resolveStructureType ty
        Nothing -> pure Map.empty
    resolveStructureType _ = pure Map.empty
...
convertExpression HLIR.MkExprNull = pure (HLIR.MkExprNull, [], HLIR.MkTyId "never")
convertExpression (HLIR.MkExprLetValueless binding inExpr) = do
  oldEnv <- readIORef locals
  modifyIORef' locals (Map.insert binding.name binding.typeValue)

  (newInExpr, ns, inTy) <- convertExpression inExpr

  writeIORef locals oldEnv

  pure
    ( HLIR.MkExprLetValueless binding newInExpr,
      ns,
      inTy
    )
...
buildMap m =
  fst $
    List.foldl'
      ...
      (HLIR.MkExprStructureEmpty, HLIR.MkTyRowEmpty)
      (Map.toList m)
...
convertPattern (HLIR.MkPatternReference p t) = do
  (newP, ns) <- convertPattern p
  pure (HLIR.MkPatternReference newP t, ns)
...
convertLambda ::
  ...
  Text ->
  m (HLIR.TLIR "expression", [HLIR.TLIR "toplevel"], HLIR.Type)
convertLambda (HLIR.MkExprLambda args _ body) reserved selfName = do
  ...
  let preEnvironment = freeVariablesInBody Map.\\ (finalNativeFunctions <> arguments)
  (ns, environment) <-
        ...
        pure
          ( HLIR.MkExprLetIn
              { HLIR.binding = HLIR.MkAnnotation name' (Identity ptr),
                ...
                HLIR.inExpr =
                  HLIR.MkExprLetIn
                    { HLIR.binding = HLIR.MkAnnotation "_" (Identity ptr),
                      HLIR.value =
                        HLIR.MkExprUpdate
                          (HLIR.MkExprDereference (HLIR.MkExprVariable (HLIR.MkAnnotation name' (Identity ptr)) []) (Identity ty))
                          (if name == selfName
                              then HLIR.MkExprNull
                              else HLIR.MkExprVariable (HLIR.MkAnnotation name (Identity ty)) []
                            )
                          (Identity ty),
  ...
  oldLocals <- readIORef locals
  modifyIORef' locals $ (<> arguments) . (<> reserved)
  ...
  let lambdaStructAlloc = malloc lambdaStructureType
      ...
      lambdaStructUpdate =
        HLIR.MkExprUpdate
          ...
          (Identity lambdaStructureType)
      lambdaStructureSelfUpdate =
        HLIR.MkExprUpdate
          ( HLIR.MkExprDereference
              ( HLIR.MkExprStructureAccess
                  (HLIR.MkExprDereference ( HLIR.MkExprStructureAccess
                      ( HLIR.MkExprDereference
                          lambdaStructAllocVar
                          (Identity lambdaStructureType)
                      )
...
convertLambda _ _ _ = M.compilerError "Expected a lambda expression"
