...
parseExprTernary = do
    ...
    (_, cond) <- Lex.parens $ snd <$> parseExprFull
    ...
    (preArgs, patterns, kwargs, kwargsType, otherKwargs) <- foldlM
        (\(accArgs, accPats, accM, accT, accOtherKwargs) param -> case param of
            ...
            LamArgNamed name ty val -> do
                let opt = isJust val
                let newT
                        | Just ty' <- ty = HLIR.MkTyRowExtend name ty' opt accT
                        | otherwise = accT
                let newM
...
parseExprList = do
    ...
    let newList  = HLIR.MkExprVarCall "List.init" [HLIR.MkExprStructureEmpty]
        ...
        addElement acc el = HLIR.MkExprFunctionAccess "push" acc [] [el, HLIR.MkExprStructureEmpty]
    ...
    Lex.locateWith
        <$> P.choice
            ...
            , P.try $ do
                ...
                pure ((start, end), HLIR.MkPatternConstructor name (args <> [HLIR.MkPatternWildcard]) Nothing)
...
parseExprFull = Lex.locateWith <$> P.makeExprParser parseExprTerm operators
  where
    ...
    parseExprArgs = do
        ...
        block <- P.optional (snd <$> (P.try parseExprLambda <|> parseExprBlock))
