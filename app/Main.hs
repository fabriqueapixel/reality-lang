...
import Language.Reality.Frontend.Parser hiding (Parser, parseError)
...
clangOptions =
  MkBuildClangOptions
    ...
    <*> App.strOption
      ( App.metavar "BACKEND"
          <> App.help "Backend used to generate the C file (e.g., 'wasm', 'native')"
          <> App.value "native"
          <> App.showDefault
          <> App.long "backend"
          <> App.short 'b'
...
buildOutput (MkBuildClangOptions inputFile _ outputFile libraries flags) = do
